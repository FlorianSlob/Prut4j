...
//        StringBuilderSyntaxHelperForJava11.addCodeInBlock(builder,"if (!"+SyntaxTreeItem.communicationChannel.queueName+".isEmpty() && "+SyntaxTreeItem.communicationChannel.queueName+".peek().getClass() == "+SyntaxTreeItem.messageContentType+".class) {", "}", tabCount,
       StringBuilderSyntaxHelperForJava11.addCodeInBlock(builder,"if (objectToGet.OriginalTargetStateId == "+SyntaxTreeItem.currentStateId+") {", "}", tabCount,
                (tabCountLvl0) -> {
                    ...
                    StringBuilderSyntaxHelper.addLine(builder, tabCountLvl0, "return Optional.of((Any)objectToGet.Message);");
                }
        );
    }
}
