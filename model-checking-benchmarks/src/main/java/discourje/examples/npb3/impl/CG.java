...
import discourje.examples.npb3.impl.CGThreads.*;
...
//---------------------------------------------------------------------
	...
	for (int m = 0; m < num_threads; m++) {
		worker[m].in.send(new discourje.examples.npb3.impl.ExitMessage());
		worker[m].out.receive();
		while (true) {
			try {
				worker[m].join();
				break;
...
//   	worker[m].notify();
//      }
		worker[m].in.send(new CGMessage(OrderNum, alpha, beta)); // TODO Place use of exchange here!
    }
    for(int m=0;m<num_threads;m++){
//      while(!worker[m].done){
