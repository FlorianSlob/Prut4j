...
public class IS extends ISBase {
    ...
    public static void main(String argv[] ){
        ...
        try{
            is = new IS(CLSS,np,serial, ProtocolHelper.GetProtocolImplementation(NpbType.IS, np,true));
...
/*This tests that keys are in sequence: sorting of last ranked key seq
  occurs here, but is an untimed operation			       */
        ...
        discourje.examples.npb3.impl.BMInOut.BMResults.printVerificationStatus(CLASS,verified,BMName);
        double tm = timer.readTimer( 0 );
        discourje.examples.npb3.impl.BMInOut.BMResults res=new BMResults(BMName,
                CLASS,
                TOTAL_KEYS,
                0,
                0,
...
//      }
        for (m = 0; m < num_threads; m++) {
            ...
            masterEnvironment.send(new RankMessage(), "worker_"+m+"_");
        }
//      for(m=0;m<num_threads;m++)
