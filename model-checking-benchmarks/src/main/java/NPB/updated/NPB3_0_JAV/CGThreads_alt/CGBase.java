...
!-------------------------------------------------------------------------!
*/
package NPB.updated.NPB3_0_JAV.CGThreads_alt;

import NPB.updated.NPB3_0_JAV.CG;
import NPB.updated.NPB3_0_JAV.CG_alt;
...
public class CGBase extends Thread{
    ...
    public static discourje.examples.npb3.impl.Timer timer;

    public CGWorker worker[];
    public CG_alt master;
    ...
    public CGBase(char clss, int np, boolean serial){
        ...
        timer = new discourje.examples.npb3.impl.Timer();
    ...
    public void setupThreads(CG_alt cg, IProtocol protocol){
        ...
        for(int i=0;i<num_threads;i++){
            ...
            try {
                worker[i] = new CGWorker(cg, start, end, protocol.getEnvironment("worker_"+i+"_") );
            } catch (Exception e) {
                e.printStackTrace();
            }
            worker[i].id=i;
