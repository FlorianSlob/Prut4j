...
import NPB.updated.NPB3_0_JAV.MGThreads.*;
...
public class MG extends MGBase {
    ...
    public static void main(String argv[] ){
        ...
        char CLSS= discourje.examples.npb3.impl.BMInOut.BMArgs.CLASS;
        int np= discourje.examples.npb3.impl.BMInOut.BMArgs.num_threads;
    ...
    public void runBenchMark() throws Exception {
        ...
        IProtocol protocol = ProtocolHelper.GetProtocolImplementation(NpbType.MG, num_threads, null);
...
//c---------------------------------------------------------------------
    ...
    public void mg3Pmaster(double u[],double v[],double r[],int n1,int n2,int n3) throws Exception {
//c---------------------------------------------------------------------
//c     multigrid V-cycle routine
//c---------------------------------------------------------------------
//      double precision u(nr),v(nv),r(nr)
...
//c---------------------------------------------------------------------
    ...
    public void residMaster(double u[],double v[],double r[],
                            int off,int n1,int n2,int n3) throws Exception {
        if (timeron) timer.start(T_resid);
...
//                resid[l].in.send(new ResidMessage(visr, 1, n3, n1, n2, n3, off));
                masterEnvironment.send(new ResidMessage(visr, 1, n3, n1, n2, n3, off));
            }
            for (int l = 0; l < num_threads; l++) {
                masterEnvironment.receive();
//                resid[l].out.receive();
    ...
    public void psinvMaster(double r[],int roffl,double u[],
                            int uoffl,int n1,int n2,int n3) throws Exception {
        if (timeron) timer.start(T_psinv);
...
//      }
            for (int l = 0; l < num_threads; l++) {
                masterEnvironment.send(new PsinvMessage(1, n3, n1, n2, n3, roffl, uoffl));
            }
            for (int l = 0; l < num_threads; l++) {
                masterEnvironment.receive();
            }
...
//	}
            for (int l = 0; l < num_threads; l++) {
                masterEnvironment.send(new InterpMessage(1, mm3, mm1, mm2, mm3, n1, n2, n3, zoffl, uoffl));
            }
            for (int l = 0; l < num_threads; l++) {
                masterEnvironment.receive();
            }
    ...
    public void rprj3Master(double r[],
                            ...
                            int soffl,int m1j,int m2j,int m3j) throws Exception {
        if (timeron) timer.start(T_rprj3);
...
//	}
            for (int l = 0; l < num_threads; l++) {
                masterEnvironment.send(new RprjMessage(2, m3j, m1k, m2k, m3k, m1j, m2j, m3j, roffl, soffl));
            }
            for (int l = 0; l < num_threads; l++) {
                masterEnvironment.receive();
            }
