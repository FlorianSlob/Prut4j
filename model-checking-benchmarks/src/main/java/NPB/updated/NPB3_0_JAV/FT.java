...
import NPB.updated.NPB3_0_JAV.FTThreads.*;
...
public class FT extends FTBase {
    ...
    public void runBenchMark() throws Exception {
        ...
        results=new discourje.examples.npb3.impl.BMInOut.BMResults(BMName,
                CLASS,
                nx,
                ny,
                nz,
...
//    }
        try{
            for (int m = 0; m < num_threads; m++) {
                environment.send(new FFTSetVariablesMessage(1, false, xtr, exp2, exp1, exp3), "fft_"+m+"_");
            }
            for (int m = 0; m < num_threads; m++) {
                environment.receive();
            }
...
//      }
            ...
            try{
                for (int m = 0; m < num_threads; m++) {
                    environment.send(new FFTSetVariablesMessage(-1, true, xnt, exp2, exp3, exp1),"fft_"+m+"_");
                }
                for (int m = 0; m < num_threads; m++) {
                    environment.receive();
                }
...
//      }
        try{
            for (m = 0; m < num_threads; m++) {
                environment.send(new FFTMessage(),"fft_"+m+"_");
            }
    //      for(m=0;m<num_threads;m++)
    //	while(!doFFT[m].done){
    ...
    public void doEvolve(int it, IEnvironment environment){
...
//      }
        ...
        for (m = 0; m < num_threads; m++) {
            environment.send(new EvolveMessage(it), "evolve_"+m+"_");
        }
//    for(m=0;m<num_threads;m++)
