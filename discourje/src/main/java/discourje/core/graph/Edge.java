...
public class Edge<State, Action, Test extends Predicate<Object> & Supplier<Object>> implements Florian {
    ...
    public String toJava() {
        var v = (clojure.lang.IPersistentVector) label.action;

        String op, type;
        switch (v.nth(0).toString()) {
            case ":send":
                op = "SEND";
                type = label.name.substring(label.name.indexOf("(") + 1, label.name.indexOf(","));
                break;
            case ":receive":
                op = "RECEIVE";
                type = "Object";
