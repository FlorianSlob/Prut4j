...
//    var m = AsyncJ.dcj() ? AsyncJ.monitor(SpecJ.session("::mg", new Object[]{num_threads})) : null;
    try {
      for (int i = 0; i < num_threads; i++) {
        ...
        psinv[i] = new Psinv(mg,protocol.getEnvironment("psinv_" + i + "_"));
        psinv[i].id = i;
        psinv[i].start();

        rprj[i] = new Rprj(mg,protocol.getEnvironment("rprj_" + i + "_"));
        rprj[i].id = i;
        rprj[i].start();

        resid[i] = new Resid(mg,protocol.getEnvironment("resid_" + i + "_"));
        resid[i].id = i;
        resid[i].start();
      }
    } catch (Exception e) {
